...
class DirectionManager {
  ...
  getSafeMoves(position) {
    ...
    return ['up', 'down', 'left', 'right'].filter((direction) => {
      ...
      // debug(direction, {x, y}, this.board.width, this.board.height);
      ...
      // debug({ noHitSnakes, noOutOfBounds });
...
const getMove = (you, board) => {
  ...
  if (weakSnakesExist && healthy && smallEnough && !tooManySnakes) {
    const weakHeads = weakSnakes.map((snake) => snake.head);
  ...
  if (foodExists && ((strongSnakesExist && smallEnough) || hungry)) {
    ...
    if (closestFoodPath) {
      ...
      // debug('nextPosition', nextPosition);
      // debug(directionManager.getSafeMoves(nextPosition));
